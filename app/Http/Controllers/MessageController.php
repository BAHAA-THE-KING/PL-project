...
class MessageController extends Controller
{
    ...
    { //reservation id
        ...
        if (
            $reservation->user_id != $connectedUserId
            && $expertsUserId != $connectedUserId
        )
            return response()->json(
                [
                    'message' => 'error',
                    'userMessage' => 'you don\'t belong to this chat'
                ],
                422
            );
    ...
    public function prepareConnection()
    {
        ...
        if (
            ...
            return response()->json(
                [
                    'message' => 'error',
                    'userMessage' => 'you dont belong to this chat'
                ],
                422
            );
        // user is good to go
        ...
        if (now()->addHours(3)->gte($expirationDate))
            return response()->json(
                [
                    'message' => 'error',
                    'userMessage' => 'reservation is done .. you cant connect to the chat'
                ],
                422
            );
        Cache::put(
            $this->getUsersReservationKey($connectedUserId),
            $connectionData,
    ...
    public function liveMessages(Request $request)
    {
        ...
        if ($connectionData == -100) {
            return response()->json(
                [
                    'message' => 'error',
                    'userMessage' => 'you are not connected to any reservation chat ... try calling prepareConnection'
                ],
                404
            );
        }
